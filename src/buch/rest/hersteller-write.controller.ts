...
export class HerstellerWriteController {
    ...
    async create(
        ...
        const result = await this.#service.create(this.#dtoToHersteller(herstellerDTO));
    ...
    @Delete(':id')
    @Roles('admin')
    @ApiOperation({ summary: 'Hersteller mit der ID löschen', tags: ['Loeschen'] })
    @ApiHeader({
        name: 'Authorization',
        description: 'Header für JWT',
        required: true,
    ...
    #handleNameExists(
        ...
        this.#logger.debug('#handleNExists(): msg=%s', msg);
        return res
