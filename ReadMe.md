...
> `create-db-hersteller.sh` und `create-db-hersteller.sql` aus dem Verzeichnis
...
> `C:\Zimmermann\volumes\postgres\tablespace\hersteller` muss außerdem Vollzugriff
...
> Datenbank-User `hersteller` und dessen Datenbank `hersteller` anlegen, d.h. der neue
> Datenbank-User `hersteller` wird zum Owner der Datenbank `hersteller`. Dazu muss man
...
> ❗ Vor dem 1. Start von MySQL muss man die Skripte `create-db-hersteller.sh` und
> `create-db-hersteller.sql` aus dem Projektverzeichnis
...
> `hersteller` und dessen Datenbank `hersteller` anlegen, d.h. der neue Datenbank-User
> `hersteller` wird zum Owner der Datenbank `hersteller`. Dazu muss man sich mit dem
...
`extras\restclient\graphql\hersteller.query.http` und
`extras\restclient\graphql\hersteller.mutation.http`.

In der Sandbox kann man z.B. folgende Query absetzen:

...
{
  hersteller(id: "000000000000000000000001") {
    titel
    art
    isbn
    version
...
```powershell
    npm exec jest --detectOpenHandles --errorOnDeprecated `
      --forceExit --runTestsByPath '__tests__\hersteller\hersteller-get.controller.test.ts'
```
...
wird das Docker-Image `docker.io/juergenzimmermann/hersteller:1.0.0` mit dem implizit
...
```powershell
    pack inspect juergenzimmermann/hersteller:1.0.0
```
...
- `helm install hersteller . -f values.yaml -f dev.yaml` in `extras\helm`
...
welches durch _Mounting_ dem Windows-Verzeichnis `C:\Zimmermann\volumes\hersteller`
...
- `helm uninstall hersteller` in `extras\helm`
...
- "Feature Filenames", z.B. hersteller.service.ts
